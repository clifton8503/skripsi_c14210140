...
def main(produk_terpilih, kode_terpilih):
    ...
    if uploaded_file_pembelian is not None and not st.session_state.uploaded:
        try:
            ...
            pembelian['Kuantitas'] = pembelian['Kuantitas'].astype(str).str.replace('.', '', regex=False).str.replace(',00', '', regex=False).astype(int)
            pembelian['Jumlah'] = pembelian['Jumlah'].astype(str).str.replace('.', '', regex=False).str.replace(',', '.', regex=False).astype(float)
            pembelian['Jumlah Data'] = pembelian['Jumlah Data'].astype(str).str.replace('.', '', regex=False).str.replace(',00', '', regex=False).astype(int)
            pembelian['Harga satuan'] = pembelian['Harga satuan'].astype(str).str.replace('.', '', regex=False).str.replace(',', '.', regex=False).astype(float)
            ...
            st.session_state.uploaded = True  # 🔒 Supaya tidak upload berulang
    ...
    temp = dfoilpp.copy(deep=True)
    # %% analisa awal hubungan oil dan pp
    from statsmodels.tsa.api import VAR
    from statsmodels.tsa.stattools import grangercausalitytests
    import matplotlib.pyplot as plt

    st.subheader("Analisis Awal Hubungan Oil dan PP")

    # === WEEKLY ===
    st.info("VAR Mingguan")
    df_var_weekly = dfoilpp.copy(deep=True)
    ...
    df_diff_weekly = df_var_weekly[['D_PP', 'D_Oil']].dropna()
    ...
    st.info(f"Minyak Bumi dan Biji Plastik PP memiliki nilai lag sebesar {optimal_lag_weekly} minggu")
    ...
    scaler = MinMaxScaler()
    temp[["Scaled_Log_PP", "Scaled_Log_Oil"]] = scaler.fit_transform(
        temp[["Price_PP", "Price_Oil"]]
    )
    import matplotlib.pyplot as plt
    ...
    sns.lineplot(data=temp, x="Date", y="Scaled_Log_PP", label="Scaled Price PP", marker="o")
    sns.lineplot(data=temp, x="Date", y="Scaled_Log_Oil", label="Scaled Price Oil", marker="s")
    ...
    correlation_matrix = temp[["Price_PP", "Price_Oil"]].corr()
    ...
    if st.button("Start"):
        ...
        # %% prediksi pp dari oil
        st.subheader("Prediksi PP dari Oil")
        ...
        def plot_pp_prediction(df_combined, X_train, X_test, y_train, y_test, model, period):
            import matplotlib.pyplot as plt

            # Buat prediksi in-sample (train)
            y_pred_train = model.predict(X_train)
            y_pred_test = model.predict(X_test)

            # Buat dataframe hasil prediksi
            df_plot = df_combined.copy()
            df_plot["Pred_PP"] = np.nan
            df_plot.loc[y_train.index, "Pred_PP"] = y_pred_train
            df_plot.loc[y_test.index, "Pred_PP"] = y_pred_test

            # Plot
            plt.figure(figsize=(12, 6))
            plt.plot(df_plot.index, df_plot["Price_PP"], label="Actual PP", color="black")
            plt.plot(df_plot.index, df_plot["Pred_PP"], label="Predicted PP", linestyle="--", color="blue")

            # Garis vertikal pisah train/test
            split_idx = y_test.index[0]
            plt.axvline(split_idx, color='red', linestyle=':', label='Train/Test Split')

            plt.title(f"[{period.upper()}] Prediksi Harga PP dari Harga Oil")
            plt.xlabel("Tanggal")
            plt.ylabel("Harga PP")
            plt.legend()
            plt.grid(True)
        ...
        def train_and_save_model(df_oil, df_pp, period):
            ...
            if os.path.exists(save_path):
                ...
                plot_pp_prediction(
                    ...
                    y_train=y_train,
                    y_test=y_test,
                    model=model_data["model"],  # atau model_data["model"] jika load dari pickle
                    period=period
                )
                return
            else:
        ...
        # %% prediksi oil dan pp future
        st.subheader("Prediksi PP dan Minyak untuk Data Future")
        ...
        def plot_prediction_xgb(df_feat, X_train, X_test, y_train, y_test, model, target_col, model_name):
            import matplotlib.pyplot as plt
            import numpy as np

            # Prediksi in-sample dan out-of-sample
            y_pred_train = model.predict(X_train)
            y_pred_test = model.predict(X_test)

            # Siapkan dataframe hasil prediksi
            df_plot = df_feat.copy()
            df_plot["Prediction"] = np.nan
            df_plot.loc[y_train.index, "Prediction"] = y_pred_train
            df_plot.loc[y_test.index, "Prediction"] = y_pred_test

            # Plot
            plt.figure(figsize=(12, 6))
            plt.plot(df_plot.index, df_plot[target_col], label="Actual", color="black")
            plt.plot(df_plot.index, df_plot["Prediction"], label="Predicted", linestyle="--", color="blue")

            # Garis pemisah train/test
            split_idx = y_test.index[0]
            plt.axvline(split_idx, color='red', linestyle=':', label='Train/Test Split')

            plt.title(f"[{model_name.upper()}] Prediksi {target_col}")
            plt.xlabel("Tanggal")
            plt.ylabel(target_col)
            plt.legend()
            plt.grid(True)
            plt.tight_layout()
            st.pyplot(plt)  # gunakan plt.show() jika bukan dalam Streamlit
